...
static u8 log_info[LOG_INFO_LEN];  //日志信息
static Bandit*  stack_bandit;  //突变算子的次数
static Bandit*  mutator_bandit[HAVOC_STACK_POW2]; //变异算子的种类
static char *bandit_log_path = NULL;     //日志存储的路径？
...
//求动态阈值，同时返回一个稀有分支列表
/*这是一个返回稀有分支的函数实现。函数首先为rare_branch_ids分配了MAX_RARE_BRANCHES个int大小的内存空间 (MAX_RARE_BRANCHES=256)
然后，它通过遍历hit_bits数组来寻找命中分支。如果找到了命中分支，并且该分支不在blacklist中，
                                         它会计算该分支的最高位，并将其与lowest_hob进行比较，以找到最低的最高位
如果最高位小于rare_branch_exp，则将该分支添加到rare_branch_ids数组中，并递增ret_list_size;
如果最高位小于rare_branch_exp-1,则将rare_branch_exp更新为最高位+1,并将ret_list_size设置为0---->移除之前添加的的分支。

如果ret_list_size=0，则表示没有找到满足条件的分支，在这种情况下，函数会检查lowest_hob是否被更新，并将rare_branch_exp设置为lowest_hob+1。
然后，它会释放之前分配的内存空间，并再次调用get_lowest_hit_branch_ids函数来重新尝试寻找满足条件的分支。

最后，函数将数组中的最后一个元素设置为-1，并返回rare_branch_ids数组的指针。

请注意，因为rare_branch_ids是通过ck_alloc分配的内存空间，因此在使用完毕后，需要调用ck_free函数来释放内存空间。
*/
...
static int* get_lowest_hit_branch_ids(){
  ...
  for (int i = 0; (i < MAP_SIZE) && (ret_list_size < MAX_RARE_BRANCHES - 1); i++){
    ...
    if (unlikely(hit_bits[i] > 0)){ //hit_bits 所有分支的命中次数
      //if (contains_id(i, blacklist)) continue; //分支在黑名单中则直接跳过
      unsigned int long cur_hits = hit_bits[i];
      int highest_order_bit = 0;
      while(cur_hits >>=1) //找大于  最小命中次数的第一个2的次幂
          highest_order_bit++;
...
static u32 * is_rb_hit_mini(u8* trace_bits_mini){
  ...
  u32 * branch_ids = ck_alloc(sizeof(u32) * MAX_RARE_BRANCHES); //命中的稀有分支列表
  u32 * branch_cts = ck_alloc(sizeof(u32) * MAX_RARE_BRANCHES); //命中次数，排序用的
  int min_hit_index = 0;

  for (int i = 0; i < MAP_SIZE ; i ++){  //遍历所有的分支？？？
      if (unlikely (trace_bits_mini[i >> 3]  & (1 <<(i & 7)) )){ //如果命中
        ...
        if (is_rare) {  //如果是稀有分支
          ...
          int j;
          for (j = 0 ; j < min_hit_index; j++){  //对命中的稀有分支列表进行排序，越稀有越靠前
  ...
  ck_free(branch_cts);  //branch_cts是用来排序的，排序后就释放掉
...
//该函数的作用是从文件中读取数据，并将读取的数据初始化到全局变量hit_bits中。这个函数通常在程序启动时调用，用于加载之前保存的分支命中数据
...
/*用来更新hit_bits
该函数在save_if_interesting中被调用，也就是每个测试用例执行完后，就会更新给hit_bits
*/
...
//该函数的功能是，将调试信息写入日志文件中。日志文件的路径为bandit_log_path
//接受一个字符串log_info作为参数，表示要记录的调试信息。
static int debug_log(char* log_info){
    //以追加模式打开名为bandit_log_path的文件
    FILE *fp = fopen(bandit_log_path, "a+");
    //将带有debug前缀的log_info写入文件
    fprintf(fp, "[DEBUG]: %s\n", log_info);
    //关闭文件。
    fclose(fp);
    return 0;
...
//bandit日志
//接受一个指向Bandit结构的指针作为参数
static int bandit_log(Bandit* bandit) {
  //生成一个字符串log_info来记录有关bandit结构的信息
